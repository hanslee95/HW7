...
CACHE_FNAME = 'tweet.json' # String for your file. We want the JSON file type, bcause that way, we can easily get the information into a Python dictionary!
...
def get(search_term):
	...
	if search_term in CACHE_DICTION:
		...
		#calling this api is different...
		x = api.search(q=search_term)	
		CACHE_DICTION[search_term] = x
		cache_file = open(CACHE_FNAME, 'w')
		#json.dumps prints out the dictionary in a json file in one line
		cache_file.write(json.dumps(CACHE_DICTION[search_term]) + "\n")
		cache_file.close()
		return CACHE_DICTION[search_term]
